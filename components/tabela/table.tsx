  ...
  const fetchData = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        toast.error("Erro ao buscar prestação de serviços!");
      }
    } catch (error) {
      console.error(error);
        ...
        <table className="w-full rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <tbody>
            ...
            <tr className="border-t border-slate-200 dark:border-slate-700">
              ...
              <td className="px-4 py-2">
                <span className="text-slate-500">waiting for estimates</span>
              </td>
            ...
            {prestacaoServico.map((item: any) => (
              ...
              >
                ...
                <td className="text-left px-4 py-2">
                  <p className="text-slate-500">{item.subtotal}</p>
                </td>

                <td className="text-left px-4 py-2">
                  <span className="text-slate-500">
                    Req: Today {item.created_at || item.updated_at}
                  </span>
                </td>
